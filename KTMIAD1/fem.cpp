...
double FEM::bc1_value(int iedge)
{
...
void FEM::getLocalRightPart(point p, double hx, double hy, double hz, int ielem)
{
    point tmp;
    int k = 0;
    F_loc.clear();
    ...
    for (int i = 0; i < F_loc_tmp.size(); i++)
      F_loc_tmp[i] = RP_value(_elems[ielem].edges[i]);

    double coef = (hx * hy * hz) / 36.0;

    for (int i = 0; i < M.size(); i++)
        for (int j = 0; j < M.size(); j++)
            F_loc[i] += coef * M[i][j] * F_loc_tmp[j];
}
...
void FEM::assemblyGlobalMatrix()
{
   ...
   for (int ielem = 0; ielem < nFE; ielem++)
   {
      ...
      getLocalG(hx, hy, hz);
      getLocalM(hx, hy, hz);
      getLocalRightPart(p1, hx, hy, hz, ielem);
    ...
    for(int i = 0; i < nz - 1; i++)
       for (int j = 0; j < ny - 1; j++)
       {
          ...
          for (int i = 0; i < 4; i+=3)
          {
             if (!isInBoundary(n[i]))
             {
                _bc1.push_back({ {n[i]}, {bc1_value(n[i])}});
             }
          }
